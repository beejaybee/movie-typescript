...
export type Cast = {
  // or do it this way [property: string]: string; when all properties are string
  character: string;
  credit_id: string;
  name: string;
  profile_path: string;
...
const apiSettings = {
  ...
  fetchActor: async (actorId: number): Promise<Actor> => {
    const actorEndpoint: string = `${API_URL}person/${actorId}?api_key=${API_KEY}&language=en-US`;
